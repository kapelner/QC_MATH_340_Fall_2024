...
\date{Due by email 11:59PM September 22, \the\year \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\intermediatesubproblem{Let $p_x = p_y$. Find $\prob{X > Y}$ in a different way by finding $\prob{X = Y}$ and then using the law of total probability.}\spc{10}

\hardsubproblem{As both $p_x$ and $p_y$ are reduced to zero, but $r = \frac{p_x}{p_y}$, what is the asymptotic probability you found in (a)? This is similar exercise to what we did in class when we took the limit of $n$ with $\lambda = np$ fixed.}\spc{10}
...
\intermediatesubproblem{Resolve as best as possible: $\int_{d}^{d+1} \indic{x \in \bracks{0, c}}\mathrm{d}x$ where $c,d \in \reals$ and $c \geq 1$.}\spc{3}

\intermediatesubproblem{Resolve as best as possible: $\int_{d}^{d+1} \indic{x \in \bracks{c, c + 1}}\mathrm{d}x$ where $c,d \in \reals$. and $c > 0$.}\spc{3}
...
\intermediatesubproblem{Let $X \sim \geometric{p}$. You can show that $\expe{X} = \frac{1-p}{p}$. Find $\var{X}$ from the definition of variance.}\spc{11}


\inthenotessubproblem{Illustrate the PDF and CDF of $X \sim \exponential{\lambda}$ one atop the other. Mark critical points on the axes.}\spc{8}


%%% need these problems here for later problems
%\easysubproblem{$X \sim \exponential{\lambda}$. Find $\expe{X}$ using $\phi_X(t) = \lambda / (\lambda - it)$.}\spc{2}
%\intermediatesubproblem{$X \sim \exponential{\lambda}$. Find $\var{X}$ using $\phi_X(t) = \lambda / (\lambda - it)$.}\spc{4}

\inthenotessubproblem{Derive the $\exponential{\lambda}$ rv from the $X_n$ Geometric r.v. with $n$ experiments compacted per time period. In the process, write the CDF, PMF and PDF of the Exponential rv. Make sure the PMF and PDF are valid $\forall x \in \reals$. The CDF you can leave in \qu{old} style.}\spc{11}
...
\intermediatesubproblem{Find the expectation of $T \sim \erlang{k}{\lambda}$. Use the expectation of the exponential rv and the linearity rules!}\spc{6}

\easysubproblem{If $X \sim \exponential{\lambda}$, then $\var{X} = 1/\lambda^2$. Find the variance of $T \sim \erlang{k}{\lambda}$. Use the variance of the exponential rv and and the linearity rules!}\spc{2}
...
\inthenotessubproblem{Prove that $\abss{e^{i \theta}} = 1$ for all $\theta$.}\spc{3}
...
\inthenotessubproblem{List ch.f. properties P0-P7 below without proofs.}\spc{10}


\intermediatesubproblem{Use P4 to compute the variance of rv $X$ using only $\phi_X(t)$ and its derivative(s).}\spc{4}

\hardsubproblem{For continuous rv $X \sim f_X(x)$, given the Fourier inversion theorem, if $\phi_X(t) \in L^1$ then prove P6. Hint: translate the Fourier formulas (the ones with $g$ and $\hat{g}$ into our vocabulary and then use $u$-substitutions.}\spc{17}

%\inblue{SOLUTION:} We begin with the statement of the forward Fourier transform. If $g \in L^1$,
%
%\beqn
%\hat{g}(\omega) = \int_\reals e^{-i2\pi\omega t} g(t) dt
%\eeqn
%
%and the statement of the inverse Fourier transform. If $\hat{g} \in L^1$,
%
%\beqn
%g(t) = \int_\reals e^{i2\pi\omega t} \hat{g}(\omega) d\omega.
%\eeqn
%
...
%
%\beqn
%\hat{g}(v)   &=& \int_\reals e^{-i2\pi v x} f(x) dx \\
%f(x)           &=& \int_\reals e^{i2\pi v x} \hat{g}(v) dv.
%\eeqn
%
...
%
%\beqn
%\hat{g}(-t / (2\pi))   &=& \int_\reals e^{i t x} f(x) dx \\
%f(x)                       &=& \int_\infty^{-\infty} e^{-i t x} \hat{g}(-t / (2\pi)) \parens{-\oneover{2\pi}} dt \\
%&=& \oneover{2\pi} \int_\reals e^{-i t x} \hat{g}(-t / (2\pi)) dt.
%\eeqn
%
%Letting $\phi_X(t) := \hat{g}(-t / (2\pi))$ completes the proof:
%
%\beqn
%\phi_X(t)   &=& \int_\reals e^{i t x} f(x) dx \\
%f(x)           &=& \oneover{2\pi} \int_\reals e^{-i t x} \phi_X(t) dt.
%\eeqn


\inthenotessubproblem{Find the ch.f. of $X \sim \bernoulli{p}$.}\spc{2.5}
...
\inthenotessubproblem{Using ch.f.'s, show that if $\Xoneton \iid \bernoulli{p}$, then $T = X_1 + \ldots + X_n \sim \binomial{n}{p}$.}\spc{5}
...
\easysubproblem{Let $X \sim \uniform{a}{b}$. Find $\phi_X(t)$.}\spc{6}
...
\easysubproblem{State the additonal assumption(s) of the CLT in addition to assumptions that were needed to prove the LLN.}\spc{1}
