...
\date{Due by email 11:59PM Sunday, September 8, \the\year \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
Find the PMF of $T = X_1 + X_2$ using a convolution. Denote it using the piecewise function notation.}\spc{20}


\easysubproblem{Derive the CDF $F_X(x)$ of $X \sim \geometric{p}$ from first principles.}\spc{4}

\easysubproblem{Derive the survival function $S_X(x)$ of $X \sim \geometric{p}$ using the result from the previous question.}\spc{1}

\hardsubproblem{Prove, using induction, the PMF of a binomial using convolutions on the sequence of r.v.'s $\Xoneton \iid \bernoulli{p}$. You will need to use Pascal's Triangle combinatorial identity we employed in class.}\spc{20}
