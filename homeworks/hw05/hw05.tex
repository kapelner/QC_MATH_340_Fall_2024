...
\date{Due by email 11:59PM November 11, \the\year \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\ingreen{
As this series of questions caused confusion in previous years, I will give you the first answer here. This answer then serves as a model answer for the next three problems as the answer will always be in this type of style.
...
\inthenotessubproblem{If $W^2 \sim F_{1, k}$ $k \in \naturals$, $W$ is symmetric about zero, then $W \sim$}\spc{-0.5}

\hardsubproblem{Let $W^2 \sim F_{1, k}$ where $k \in \naturals$. Find the PDF of $Y = \sqrt{W^2}$ by using transformation of variables.} \spc{10}
...
\problem{We will now practice multivariate change of variables where $\Y = \bv{g}(\X)$ where $\X$ de
 a vector of $k$ continuous r.v.'s and $\bv{g} : \reals^k \rightarrow \reals^k$ and is 1:1.}
