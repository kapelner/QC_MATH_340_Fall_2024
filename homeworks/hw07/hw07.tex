...
\date{Due by email 11:59PM Dec 14 \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\intermediatesubproblem{Let $Z \sim \stdnormnot$ and $Y = Z\,\big|\,|Z|>2}$. Find $f_Y(y)$. Note: $\prob{|Z|>2} \approx 5\%$.}\spc{2}


\inthenotessubproblem{If $X \sim \text{ParetoI}(k, \lambda)$ and $c > 0$. Prove $Y = X\,|\,X > k +c$ is also ParetoI-distributed and find its density.}\spc{6}
...
\inthenotessubproblem{Using the formula we derived in class for $\lambda$ as a ratio of logs, find the ParetoI distribution corresponding to the 80-20 rule. The value of $k$ doesn't matter; let $k=1$.}\spc{3}

\easysubproblem{Let $X \sim \text{ParetoI}(\lambda, 1)$ where the value of $\lambda$ was found in the previous question. Find $Q[X, 0.8]$, the 80th \%ile of this distribution.}\spc{3}
...
\easysubproblem{If human wealth follows the 99-1 rule, what do the top 1\% own?}\spc{3}

\intermediatesubproblem{If human wealth follows the 99-1 rule, what do the top top 1\% of the top 1\% own?}\spc{3}
\easysubproblem{Plot the PDF from (b) in the range $x \in \bracks{0, 20}$ and shade in the area where $x <a$ in blue and $x \geq a$ in red. Label the axes appropriately. \inblue{This problem is done for you (see Figure 1 next page).}}

\begin{figure}[htp]
\centering
\includegraphics[width=5in]{pareto_density.pdf}
\caption{PDF of the Pareto from (a)}
\end{figure}

\easysubproblem{Plot the PDF from (b) times $x$ (this is the total amount of land owned by people up to land amount $x$) in the range $x \in \bracks{0, 20}$ and shade in the area where $x <a$ in blue and $x \geq a$ in red. Label the axes appropriately. \inblue{This problem is done for you (see Figure 2 next page).}}

\begin{figure}[htp]
\centering
\includegraphics[width=5in]{pareto_density_times_x.pdf}
\caption{PDF of the integral of the Pareto PDF from (a) times $x$}
\end{figure}
...
\inblue{Although the plot of the density has 80\% of the total area in the region where $x<a$, the plot of the total amount of land owned has only 20\% of the total area in the region where $x<a$. This is the 80-20 rule.}\spc{2}
...
\inthenotessubproblem{Draw the densities of the three types of Weibull distributions and indicate the set of the Weibull modulus $k$ below each graph.}\spc{6}

\inthenotessubproblem{Let $X \sim \text{Weibull}(k, \lambda)$. If $k = 1$, prove that this Weibull is an exponential distribution with rate paramter $\lambda$ and then prove that it exhibits the memorlessness property.}\spc{4}
...
\inthenotessubproblem{Let $X \sim \text{Weibull}(k, \lambda)$. If $k \in (0, 1)$, prove that $\cprob{X > x + c}{X > c} > \prob{X > x}$ for all $x, c > 0$. This proof follows from the previous question. Begin at Inequality~\ref{eq:weibull_inequality} and flip the inequality.}\spc{9}

\intermediatesubproblem{Let $X \sim \text{Weibull}(k, \lambda)$ where $k > 1$. Provide one real life example that could be modeled by this rv that was not the one we said in class.}\spc{3}

\intermediatesubproblem{Let $X \sim \text{Weibull}(k, \lambda)$ where $k \in (0, 1)$. Provide one real life example that could be modeled by this rv that was not the one we said in class.}\spc{3}

\end{enumerate}


\problem{We will acquaint ourselves with the law of iterated expectation and the law of total variance.}
...
\intermediatesubproblem{[MA] If $X \sim \gammanot{\alpha}{\beta}$ and $Y\,|\,X =x \sim \exponential{x}$, find $\var{Y}$ using the law of total variance.}\spc{5}

\intermediatesubproblem{This is a canonical problem in econometrics. If $X \sim \normnot{\mu}{\tau^2}$ and $Y\,|\,X =x \sim \normnot{a + bx}{\sigsq}$, where $a, b \in \reals$, find $\var{Y}$ using the law of total variance.}\spc{4}
...
\easysubproblem{Using your answers to parts (b-d), write an iterative algorithm that can draw samples from $\X$.}\spc{9}
...
\easysubproblem{State properties 0-9 for joint ch.f's.}\spc{11}

\inthenotessubproblem{Let $\X \sim \text{Mult}_K(n, \p)$. Find $\phi_{\X}(\t)$.}\spc{6}


\easysubproblem{Find the distribution of $X_{17}$ using $\phi_{\X}(\t)$.}\spc{3}

\intermediatesubproblem{[MA] Find $\cov{X_{17} X_{37}}{X_{53}}$ using $\phi_{\X}(\t)$.}\spc{6}
...
\easysubproblem{Find $\phi_{\Z}(\t)$. Hint: use the iid property and the known chf of the standard normal.}\spc{4}

\easysubproblem{Let $A \in \reals^{m \times n}$ and $\mu \in \reals^n$. Let $\X = \muvec + A\Z$. Let $\bSigma := AA^\top$, Find $\phi_{\X}(\t)$.}\spc{6}
...
\intermediatesubproblem{Assuming for any vector rv $\Y$ with dimension $n$ that $\var{\muvec + \Y} = \var{\T}$ and $\var{A\Y} = A\var{\Y}A^\top$. Let $\X \sim \multnormnot{n}{\muvec}{\bSigma}$.  Show that $\var{\X} = \bSigma$.}\spc{5}

\inthenotessubproblem{Define the Mahalanobis distance. How is it distributed?}\spc{2}
...
\hardsubproblem{[MA] Are there any restrictions on $\muvec, \c, B, \bSigma$ in the previous question to have the result you found?}\spc{4}
...
\intermediatesubproblem{Let $X \sim \text{Gamma}(\alpha, \beta)$. Find Mode[$X$]. Any restrictions on $\alpha, \beta$ besides the parameter space?}\spc{5}

\intermediatesubproblem{Let $X \sim T_{k}$. Let $Y = \mu + \sigma X \sim T_{k}(\mu, \sigsq)$ where $\mu \in \reals$ and $\sigma > 0$ which is called the \qu{location-scale T-distribution}. Find Mode[$Y$]. We can use the result from a previous year's midterm II:
\beqn
...
\intermediatesubproblem{Let $N \sim \poisson{\lambda}$. Describe a way to use the realizations from the r.v.'s $X_1, X_2, X_3, \ldots \iid \exponential{\lambda}$ to create a realization $n$ from the Poisson model.}\spc{5}
