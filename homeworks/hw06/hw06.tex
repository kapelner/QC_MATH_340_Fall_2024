...
\date{Due by email 11:59PM November 25, \the\year \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\inthenotessubproblem{Find the kernel of $X \sim \binomial{n}{p}$.}\spc{3}

\inthenotessubproblem{Find the kernel of $X \sim \text{Mult}_k\parens{n,\p}$.}\spc{2}

\easysubproblem{Find the kernel of $X \sim \gammanot{\alpha}{\beta}$.}\spc{2}

\easysubproblem{Find the kernel of $X \sim \text{BetaPrime}\parens{\alpha, \beta}$.}\spc{2}

\easysubproblem{Find the kernel of $\text{Cauchy}\parens{\mu,\sigma}$.}\spc{2}
...
\easysubproblem{Find the kernel of the PDF of $X \sim \text{Log}\normnot{\mu}{\sigsq}$.}\spc{1}

\intermediatesubproblem{If $k(x) = e^{-\lambda x} x^{k-1} \indic{x > 0}$ how would you know if the r.v. $X$ was an $\text{Erlang}\parens{k, \lambda}$ or a $\gammadist{k}{\lambda}$?}\spc{1}

\intermediatesubproblem{If $k(x) = e^{ax - bx^2}$, solve for the normalization constant $c$.}\spc{2}


\intermediatesubproblem{If $k(x) = xe^{-x^2} \indic{x > 0}$, how is $X$ distributed?}\spc{1}

\inthenotessubproblem{Fill in the integral definition. $B(\alpha, \beta) := $}\spc{-0.5}

\inthenotessubproblem{Prove $B(\alpha, \beta) = \frac{\Gammaf{\alpha}\Gammaf{\beta}}{\Gammaf{\alpha+\beta}}$ using the method from class.}\spc{11}


\easysubproblem{Find the kernel of the PDF of $X \sim T_\nu$.}\spc{2}
...
\intermediatesubproblem{If discrete $\Xoneton \iid $ some discrete rv, why would the formula in (b) not be correct?}\spc{3}
...
\easysubproblem{Prove that $\betanot{1}{1} = \stduniform$.}\spc{1}
...
\hardsubproblem{If $X \sim \binomial{n}{p}$, show that $F(x) = I_{1-p}(n-k, k+1)$. You will need to assume the property $I_x(a,b) = 1 - I_{1-x}(b,a)$.}

\inblue{This answer is done for you. See last page of the homework so you can copy it.}\spc{20}
...
\inthenotessubproblem{Let $X$ be a continous rv. Prove $Y = F_X(X) \sim \stduniform$.}\spc{6}
...
\intermediatesubproblem{[MA] Let $X_1, X_2 \iid \binomial{n}{p}$ and $T = X_1 + X_2$. Show that $X_1~|~T = t$ is hypergeometric. You can find information about this r.v. online. Note we did not / will not study the hypergeometric further and it will not be covered on any exams.}\spc{5}
...
\easysubproblem{If $X \sim \betanot{\alpha}{\beta}$ and $Y\,|\,X = x \sim \binomial{n}{x}$, find the PMF of $Y$.}\spc{6}


\hardsubproblem{If $X \sim \gammanot{\alpha}{\beta}$ and $Y\,|\,X = x \sim \exponential{x}$, show that $Y \sim \text{Lomax}(\alpha, \beta)$. See the Lomax's Wikipedia article  by clicking \href{https://en.wikipedia.org/wiki/Lomax_distribution}{here}. Kernels always make it easier.}\spc{10}
...
%\begin{enumerate}
%
%\intermediatesubproblem{Let $X \sim \text{Gamma}(\alpha, \beta)$. Find Mode[$X$]. Any restrictions on $\alpha, \beta$ besides the parameter space?}\spc{4}
%
%\intermediatesubproblem{Let $X \sim \text{InvGamma}(\alpha, \beta) := \frac{\beta^\alpha}{\Gamma\parens{\alpha}} x^{-\alpha - 1} e^{-\frac{\beta}{x}} \indic{x \in (0, \infty)}$. Find Mode[$X$]. Any restrictions on $\alpha, \beta$ besides the parameter space (which is idential to the Gamma distribution's parameter space)?}\spc{4}
%
%\intermediatesubproblem{Let $X \sim T_{k}$. Let $Y = \mu + \sigma X$ where $\mu \in \reals$ and $\sigma > 0$. We proved on the 2023 midterm II that $Y \sim T_{k}(\mu, \sigsq)$ with density:
%
%\beqn
...
\section*{Solution for the Binomial CDF question}
