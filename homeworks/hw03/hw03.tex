...
\hardsubproblem{Let $X_n \sim \exponential{n}$. Compute upper bounds for $\prob{X \geq 3}$ via Markov and Chebyshev. Does one go to zero \qu{faster} than the other? Explain.}\spc{10}
...
Show that $X_n \convd 0$. You can use the theorem we never proved that $\limitn \indic{a_n} = \indic{\limitn a_n}$.}\spc{4}
